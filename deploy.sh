...
# 2. 备份
echo -e "${YELLOW}📦 步骤1/5: 备份现有数据...${NC}"
mkdir -p "$BACKUP_DIR"
cp -r "$APP_DIR/data" "$BACKUP_DIR/" 2>/dev/null || true
cp -r "$APP_DIR/server" "$BACKUP_DIR/" 2>/dev/null || true
cp "$APP_DIR/src/index.html" "$BACKUP_DIR/index.html.bak" 2>/dev/null || \
cp "$APP_DIR/public/index.html" "$BACKUP_DIR/index.html.bak" 2>/dev/null || \
cp "$APP_DIR/frontend/index.html" "$BACKUP_DIR/index.html.bak" 2>/dev/null || true
echo -e "${GREEN}✅ 备份完成: $BACKUP_DIR${NC}"
echo ""

# 3. 写入重心版 index.html
echo -e "${YELLOW}📝 步骤2/5: 写入重心版前端...${NC}"
mkdir -p "$APP_DIR/src"
mkdir -p "$APP_DIR/public"
mkdir -p "$APP_DIR/frontend"

cat > "$APP_DIR/src/index.html" << 'HTML_EOF'
<!DOCTYPE html>
<html lang="zh">
<head>
<meta charset="UTF-8">
...
--text:#e6edf3;--text2:#8b949e;--primary:#58a6ff;--primary2:#1f6feb;
--green:#3fb950;--purple:#a371f7;--orange:#f0883e;--yellow:#d29922;
--teal:#39c5cf;--blue:#58a6ff;--red:#f85149;--pink:#db61a2;
}
...
.avatar{width:40px;height:40px;border-radius:50%;background:var(--primary);display:flex;align-items:center;justify-content:center;cursor:pointer;overflow:hidden}
...
.center-card{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:16px;cursor:pointer;transition:0.2s;position:relative;overflow:hidden}
...
.center-card .progress-bar{height:100%;background:linear-gradient(90deg,var(--primary),var(--purple));border-radius:2px;transition:0.3s}
...
.module-item{background:var(--card);border:1px solid var(--border);border-radius:14px;padding:14px 16px;display:flex;align-items:center;gap:14px;cursor:pointer;transition:0.15s}
...
.report-card p{font-size:13px;color:var(--text2);line-height:1.6;margin-bottom:8px}
...
.nav-item{flex:1;text-align:center;padding:6px;cursor:pointer;color:var(--text2);transition:0.2s}
...
.list-item .right .date{font-size:11px;color:var(--text2)}
...
.submodule-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-top:8px}
.submodule-item{background:var(--card2);border:1px solid var(--border);border-radius:10px;padding:10px;text-align:center;font-size:12px;cursor:pointer}
.submodule-item:active{background:var(--border)}
</style>
</head>
<body>
...
<div id="home" class="tab-page active">
  <div class="header">
    <h1>🍆 茄子管家</h1>
    <div class="avatar" id="avatarBtn"><i class="fas fa-user"></i></div>
  </div>

  <div class="balance-card" id="balanceCard">
    <div class="label">平衡指数</div>
    <div class="score" id="balanceScore">--</div>
    <div class="desc" id="balanceDesc">保持觉察，稳步前行</div>
  </div>

  <div class="quick-input">
    <i class="fas fa-microphone"></i>
    <input type="text" id="quickInput" placeholder="说一句：今晚11点睡/记账午餐35块/今天很累7分">
  </div>
...
<div id="focus" class="tab-page">
  ...
  <div class="balance-card">
    <div class="label">综合平衡指数</div>
    <div class="score" id="focusScore">--</div>
    <div class="desc" id="focusDesc">最近7天综合表现</div>
  </div>
  <div class="section-title"><i class="fas fa-chart-bar"></i> 三维统计</div>
  <div class="focus-stats" id="focusStats">
    <div class="stat-card"><div class="num" style="color:var(--purple)" id="statSleep">--</div><div class="label">睡眠(h)</div></div>
    <div class="stat-card"><div class="num" style="color:var(--yellow)" id="statEmotion">--</div><div class="label">情绪</div></div>
    <div class="stat-card"><div class="num" style="color:var(--orange)" id="statExercise">--</div><div class="label">锻炼(次)</div></div>
  </div>
...
<div id="message" class="tab-page">
  ...
  <div class="chat-input">
    <input type="text" id="chatInput" placeholder="说点什么...（如：记账午餐35块）">
...
<div id="profile" class="tab-page">
  ...
  <div class="profile-section">
    <div class="profile-item" onclick="uploadAvatar()">
      <i class="fas fa-camera"></i><div class="name">修改头像</div><span class="arrow"><i class="fas fa-chevron-right"></i></span>
    </div>
    <div class="profile-item" onclick="showStats()">
      <i class="fas fa-chart-pie"></i><div class="name">数据统计</div><span class="arrow"><i class="fas fa-chevron-right"></i></span>
    </div>
    <div class="profile-item" onclick="backupData()">
      <i class="fas fa-cloud-arrow-down"></i><div class="name">数据备份</div><span class="arrow"><i class="fas fa-chevron-right"></i></span>
    </div>
    <div class="profile-item" onclick="clearCache()">
      <i class="fas fa-trash"></i><div class="name">清除缓存</div><span class="arrow"><i class="fas fa-chevron-right"></i></span>
    </div>
  </div>
</div>
...
<div class="bottom-nav">
  ...
  <div class="nav-item" data-tab="message"><i class="fas fa-comments"></i><span>消息</span></div>
  <div class="nav-item" data-tab="profile"><i class="fas fa-user"></i><span>我的</span></div>
</div>

<script>
var AUTH_KEY='qiezi_token';
var authToken=localStorage.getItem(AUTH_KEY)||'';

// 4个重心
var CENTERS=[
  {id:'work',name:'工作收入',icon:'fa-briefcase',color:'var(--blue)',desc:'财务+职业',modules:['finance','career']},
  {id:'photo',name:'摄影能力',icon:'fa-camera',color:'var(--pink)',desc:'技能+作品',modules:['skill']},
  {id:'health',name:'身体健康',icon:'fa-heart-pulse',color:'var(--green)',desc:'睡眠+锻炼+饮食',modules:['sleep','exercise','diet']},
  {id:'cognition',name:'认知思考',icon:'fa-brain',color:'var(--purple)',desc:'情绪+日记+复盘',modules:['emotion','diary','learn']}
];

// 6个核心模块
var MODULES=[
  {id:'finance',label:'财务',icon:'fa-wallet',color:'var(--green)',desc:'记账与收支管理',
    fields:[
      {key:'date',label:'日期',type:'date'},
      {key:'type',label:'收支类型',type:'select',options:['支出','收入']},
      {key:'category',label:'分类',type:'select',options:['餐饮','交通','购物','学习','医疗','提成','其他']},
      {key:'amount',label:'金额(元)',type:'number'},
      {key:'note',label:'备注',type:'textarea'}
    ]},
  {id:'sleep',label:'睡眠',icon:'fa-moon',color:'var(--purple)',desc:'睡眠时长与质量',
    fields:[
      {key:'date',label:'日期',type:'date'},
      {key:'hours',label:'睡眠时长(小时)',type:'number'},
      {key:'quality',label:'质量评分(1-5)',type:'select',options:['1','2','3','4','5']},
      {key:'note',label:'备注',type:'textarea'}
    ]},
  {id:'exercise',label:'锻炼',icon:'fa-dumbbell',color:'var(--orange)',desc:'运动记录',
    fields:[
      {key:'date',label:'日期',type:'date'},
      {key:'type',label:'运动类型',type:'text'},
      {key:'duration',label:'时长(分钟)',type:'number'},
      {key:'intensity',label:'强度',type:'select',options:['轻度','中度','高强度']}
    ]},
  {id:'emotion',label:'情绪',icon:'fa-face-smile',color:'var(--yellow)',desc:'情绪评分',
    fields:[
      {key:'date',label:'日期',type:'date'},
      {key:'score',label:'情绪评分(1-10)',type:'number'},
      {key:'tags',label:'情绪标签',type:'text'},
      {key:'note',label:'记录',type:'textarea'}
    ]},
  {id:'diary',label:'日记',icon:'fa-book-open',color:'var(--blue)',desc:'每日记录',
    fields:[
      {key:'date',label:'日期',type:'date'},
  ...
  {id:'learn',label:'学习复盘',icon:'fa-graduation-cap',color:'var(--purple)',desc:'成长记录',
    fields:[
      {key:'date',label:'日期',type:'date'},
      {key:'topic',label:'学习主题',type:'text'},
      {key:'insight',label:'收获/洞察',type:'textarea'},
      {key:'action',label:'下一步行动',type:'textarea'}
    ]}
];

// 饮食子模块（健康重心的子模块）
var DIET_SUBS=[
  {key:'meals',label:'餐数',icon:'fa-utensils'},
  {key:'fruits',label:'水果',icon:'fa-apple-whole'},
  {key:'snacks',label:'零食',icon:'fa-cookie-bite'},
  {key:'coffee',label:'咖啡',icon:'fa-mug-hot'}
];

// ---------- API ----------
function apiFetch(url,opts){
  opts=opts||{};opts.headers=opts.headers||{};
  opts.headers['x-anonymous-token']=authToken;
  opts.headers['Content-Type']='application/json';
  return fetch(url,opts).then(function(r){
    var t=r.headers.get('x-anonymous-token');
    if(t&&t!==authToken){authToken=t;localStorage.setItem(AUTH_KEY,t);}
    return r;
  });
}
...
document.querySelectorAll('.nav-item').forEach(function(n){
  n.onclick=function(){switchTab(this.dataset.tab)};
});
...
function renderHome(){
  ...
  CENTERS.forEach(function(c){
    ...
    card.innerHTML='<div class="icon-wrap" style="background:'+c.color+'"><i class="fas '+c.icon+'"></i></div>'+
  ...
  MODULES.forEach(function(m){
    ...
    item.innerHTML='<div class="icon-wrap" style="background:'+m.color+'"><i class="fas '+m.icon+'"></i></div>'+
      '<div class="info"><div class="name">'+m.label+'</div><div class="sub">'+m.desc+'</div></div>'+
...
function openModule(id){
  ...
  // 饮食子模块
  var sub=document.getElementById('subModules');
  if(id==='diet'||id==='health'){
    sub.innerHTML='<div class="section-title"><i class="fas fa-utensils"></i> 饮食记录</div><div class="submodule-grid">'+
      DIET_SUBS.map(function(s){return '<div class="submodule-item"><i class="fas '+s.icon+'"></i><br>'+s.label+'</div>'}).join('')+'</div>';
  } else {
    sub.innerHTML='';
  }
  loadRecords();
...
function renderList(){
  ...
  list.innerHTML=currentData.slice().reverse().map(function(r,i){
...
document.getElementById('saveBtn').onclick=function(){
  ...
  apiFetch('/api/'+currentModule,{method:'POST',body:JSON.stringify(currentData)}).then(function(r){return r.json()}).then(function(){
...
function parseQuickInput(text){
  ...
  if(m1){
    return {module:'finance',data:{date:new Date().toISOString().slice(0,10),type:'支出',category:m1[1].trim()||'其他',amount:m1[2],note:text}};
  }
  // 睡眠: 睡X小时 / 昨晚睡7小时
  var m2=text.match(/睡[着了]?(\d+(?:\.\d+)?)\s*(?:小时|h|个?钟头)/);
  if(m2){
    return {module:'sleep',data:{date:new Date().toISOString().slice(0,10),hours:m2[1],quality:'4',note:text}};
  }
  // 情绪: 心情X分 / X分
  var m3=text.match(/[今昨]?天?(?:心情|情绪)?(\d+)\s*分/);
  if(m3){
    return {module:'emotion',data:{date:new Date().toISOString().slice(0,10),score:m3[1],tags:text,note:text}};
  }
  return null;
}
...
document.getElementById('quickInput').onkeypress=function(e){
  ...
  if(!text)return;
  var parsed=parseQuickInput(text);
  if(parsed){
    apiFetch('/api/'+parsed.module,{method:'POST',body:JSON.stringify([parsed.data])}).then(function(){return apiFetch('/api/'+parsed.module)}).then(function(r){return r.json()}).then(function(arr){
      var data=Array.isArray(arr)?arr:[];
      data.push(parsed.data);
      return apiFetch('/api/'+parsed.module,{method:'POST',body:JSON.stringify(data)});
    }).then(function(){showToast('已记录到'+parsed.module);this.value='';}.bind(this)).catch(function(){showToast('记录失败')});
  } else {
    showToast('已识别为：'+text);
    this.value='';
  }
...
function loadFocus(){
  Promise.all([
    apiFetch('/api/sleep').then(function(r){return r.json()}),
    apiFetch('/api/emotion').then(function(r){return r.json()}),
    apiFetch('/api/exercise').then(function(r){return r.json()})
  ]).then(function(arr){
  ...
  apiFetch('/api/state').then(function(r){return r.json()}).then(function(data){
    ...
    if(data.insights){
      html+='<div class="report-card"><h3><i class="fas fa-lightbulb"></i> 30天洞察</h3>'+
        data.insights.map(function(i){return '<p>· '+i+'</p>'}).join('')+'</div>';
    } else {
      html+='<div class="report-card"><h3><i class="fas fa-seedling"></i> 开始记录</h3><p>继续记录数据，30天后将自动生成阶段审视报告。</p></div>';
    }
    html+='<div class="report-card"><h3><i class="fas fa-bullseye"></i> 下一步建议</h3>'+
      '<span class="tag">保持睡眠≥7小时</span>'+
      '<span class="tag">每天记录情绪</span>'+
      '<span class="tag">每周3次锻炼</span>'+
      '<span class="tag">财务月度复盘</span></div>';
    document.getElementById('reportContent').innerHTML=html;
  }).catch(function(){
    document.getElementById('reportContent').innerHTML='<div class="report-card"><h3><i class="fas fa-info-circle"></i> 暂无数据</h3><p>开始记录后将自动生成报告。</p></div>';
  });
}
...
document.getElementById('chatSend').onclick=function(){
  ...
  apiFetch('/api/voice',{method:'POST',body:JSON.stringify({text:text})}).then(function(r){return r.json()}).then(function(d){
    ...
    if(d.action==='add_record'){
      apiFetch('/api/'+d.module).then(function(r){return r.json()}).then(function(arr){
        var data=Array.isArray(arr)?arr:[];
        data.push(d.data);
        return apiFetch('/api/'+d.module,{method:'POST',body:JSON.stringify(data)});
      });
    }
  }).catch(function(){addChat('处理失败，请重试',false)});
};

// ---------- 我的页 ----------
function uploadAvatar(){
  var url=prompt('请输入头像URL（或留空上传文件）:');
  if(!url)return;
  apiFetch('/api/avatar',{method:'POST',body:JSON.stringify({url:url})}).then(function(){showToast('头像已更新')});
}
function showStats(){
  Promise.all(MODULES.map(function(m){return apiFetch('/api/'+m.id).then(function(r){return r.json()}).then(function(d){return [m.label,(Array.isArray(d)?d.length:0)]})})).then(function(arr){
    var msg=arr.map(function(x){return x[0]+': '+x[1]+' 条'}).join('\n');
    alert('📊 数据统计\n\n'+msg);
  });
}
function backupData(){
  apiFetch('/api/backup').then(function(){showToast('备份已创建')}).catch(function(){showToast('备份失败')});
}
function clearCache(){
  if(confirm('确认清除本地缓存？数据不会丢失。')){
    localStorage.clear();
    showToast('缓存已清除');
...
apiFetch('/api/state').then(function(r){return r.json()}).then(function(d){
  if(d&&d.balance!==undefined){
    document.getElementById('balanceScore').textContent=d.balance;
    document.getElementById('balanceDesc').textContent=d.desc||'保持觉察，稳步前行';
  }
}).catch(function(){});

// 头像加载
apiFetch('/api/avatar').then(function(r){return r.json()}).then(function(d){
  if(d&&d.url){
    document.getElementById('avatarBtn').innerHTML='<img src="'+d.url+'">';
  }
}).catch(function(){});
</script>
</body>
...
# 同步到 public 和 frontend
cp "$APP_DIR/src/index.html" "$APP_DIR/public/index.html"
cp "$APP_DIR/src/index.html" "$APP_DIR/frontend/index.html"

echo -e "${GREEN}✅ 重心版前端已写入${NC}"
echo ""

# 4. 保留数据
echo -e "${YELLOW}💾 步骤3/5: 保留历史数据...${NC}"
if [ -d "$APP_DIR/data" ]; then
    echo -e "${GREEN}✅ 数据目录存在: $APP_DIR/data${NC}"
    echo "数据文件:"
    ls -la "$APP_DIR/data" 2>/dev/null | head -20
else
...
# 5. 重启服务
echo -e "${YELLOW}🔄 步骤4/5: 重启服务...${NC}"
if systemctl is-active --quiet qiezi 2>/dev/null; then
    sudo systemctl restart qiezi
    echo -e "${GREEN}✅ systemd 服务 qiezi 已重启${NC}"
    ...
    sleep 1
    cd "$APP_DIR/server" 2>/dev/null || cd "$APP_DIR"
    nohup node index.js > /tmp/qiezi.log 2>&1 &
    echo -e "${GREEN}✅ Node服务已启动 (PID: $!)${NC}"
fi
echo ""

# 6. 健康检查
echo -e "${YELLOW}🏥 步骤5/5: 健康检查...${NC}"
sleep 3
HEALTH=$(curl -s http://localhost:3000/api/health 2>&1 || curl -s http://localhost:3000/api/health-check 2>&1)
if [ -n "$HEALTH" ]; then
    echo -e "${GREEN}✅ 服务响应: $HEALTH${NC}"
else
    echo -e "${RED}⚠️ 服务可能未完全启动，请稍后检查${NC}"
fi
echo ""
...
echo "========================================"
echo ""
echo "📱 访问: http://49.235.185.200"
echo "💾 备份: $BACKUP_DIR"
echo ""
echo "💡 建议："
echo "   1. 浏览器无痕模式打开 http://49.235.185.200"
echo "   2. 如显示旧版，清除浏览器缓存后刷新"
echo "   3. 4个重心：工作收入/摄影能力/身体健康/认知思考"
echo "   4. 6个核心模块：财务/睡眠/锻炼/情绪/日记/学习复盘"
echo ""
