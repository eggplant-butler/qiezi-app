// FIX8b: 追加智慧透镜三函数到 app.js 末尾（避免单引号转义问题）
const fs = require('fs');
...
function openWisdomLens(){
  ...
  html += "<div style=\\"display:grid;grid-template-columns:1fr 1fr;gap:8px\\" id=\\"wisdomLensGrid\\">";
  WISDOM_LENS_LIST.forEach(function(l){
    html += "<div data-lens=\\""+l.key+"\\" data-name=\\""+l.name+"\\" style=\\"background:var(--c-surface);border:1px solid var(--c-input-border,#F5F7FA);border-radius:10px;padding:12px 10px;cursor:pointer;text-align:left\\">";
  ...
  // 事件委托：避免 onclick 内联单引号转义
  setTimeout(function(){
    var grid = document.getElementById("wisdomLensGrid");
    if (!grid) return;
    grid.querySelectorAll("[data-lens]").forEach(function(el){
      el.onclick = function(){
        var k = el.getAttribute("data-lens");
...
function openCrisisFramework(){
  ...
  html += "<button id=\\"crisisRunBtn\\" style=\\"width:100%;margin-top:10px;padding:12px;border:none;border-radius:10px;background:linear-gradient(135deg,#EF4444,#F87171);color:#fff;font-weight:600;cursor:pointer\\">🛟 开始危机应对分析</button>";
  html += "<div id=\\"crisisResult\\" style=\\"margin-top:12px\\"></div>";
  openModal2("🛟 危机应对框架", html);
  setTimeout(function(){
    var btn = document.getElementById("crisisRunBtn");
    if (btn) btn.onclick = runCrisisFramework;
  }, 50);
}
